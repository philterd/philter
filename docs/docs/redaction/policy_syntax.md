...
*   **`FPE` (Format-Preserving Encryption)**: Encrypts the sensitive data while ensuring the output has the same format and length as the input. This is ideal for maintaining database schema compatibility. No configuration is required: Philter manages a stable key for each user automatically, so the same input always encrypts to the same value (referential integrity / join compatibility across documents) and the value can be recovered with the user's key. To use your own key instead, supply a key and tweak in the policy's top-level `fpe` object.
